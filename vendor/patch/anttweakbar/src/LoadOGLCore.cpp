...
//  ---------------------------------------------------------------------------
    ...
    int LoadOpenGLCore() 
    {
        ...
        if( _glBindVertexArray==NULL || _glDeleteVertexArrays==NULL || _glGenVertexArrays==NULL || _glIsVertexArray==NULL )
        {
            ...
            if( _glBindVertexArray==NULL || _glDeleteVertexArrays==NULL || _glGenVertexArrays==NULL || _glIsVertexArray==NULL )
			{
                fprintf(stderr, "AntTweakBar: OpenGL Core Profile functions cannot be loaded.%p%p%p%p\n", _glBindVertexArray, _glDeleteVertexArrays, _glGenVertexArrays, _glIsVertexArray);
                return 0;                
			}
        }
